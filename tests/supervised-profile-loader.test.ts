...
describe('loadSupervisedProfile', () => {
  it('loads a valid v1 profile and returns a stable hash', async () => {
    ...
    expect(result.profile.knowledge.enabled).toBe(false);
    expect(result.profile.knowledge.max_bytes).toBe(80000);
  ...
  it('validates knowledge include paths and byte cap shape', async () => {
    const badPath = await writeProfile('bad-path', `${VALID_PROFILE}\nknowledge: { enabled: true, include: [../outside.md], max_bytes: 80000 }\n`);
    const badPathResult = await loadSupervisedProfile('bad-path', { homeDir: badPath });
    expect(badPathResult.ok).toBe(false);
    if (badPathResult.ok) throw new Error('expected failure');
    expect(badPathResult.error.field).toBe('knowledge.include.0');

    const badMax = await writeProfile('bad-max', `${VALID_PROFILE}\nknowledge: { enabled: true, include: [docs/specs/*.md], max_bytes: -1 }\n`);
    const badMaxResult = await loadSupervisedProfile('bad-max', { homeDir: badMax });
    expect(badMaxResult.ok).toBe(false);
    if (badMaxResult.ok) throw new Error('expected failure');
    expect(badMaxResult.error.field).toBe('knowledge.max_bytes');
  });
